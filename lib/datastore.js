...
function MongoDataStore(config) {
  ...
  if (config.internalReconnectionPolicy) {

    config.opts.bufferMaxEntries = 0;//enforce an immediately returned error when we cannot access the db
    config.opts.autoReconnect = false;
  } else {

    if (!config.opts.reconnectTries) config.opts.reconnectTries = 300;//five minutes
  }
...
MongoDataStore.prototype.connect = function (callback) {
  ...
  async.whilst(function () {
    ...
    mongoclient.connect(_this.config.url, _this.config.opts, function (err, client) {

      if (err) return setTimeout(next, 1000);

      _this.connection = client;
      _this.db = client.db(_this.config.database);
      _this.data = _this.db.collection(_this.config.collection);

      _this.CONNECTED = true;

      _this.connection.on('close', _this.__handleDisconnected.bind(_this));
      _this.connection.on('reconnected', _this.__handleReconnected.bind(_this));
...
MongoDataStore.prototype.__handleDisconnected = function () {
  ...
  if (this.config.internalReconnectionPolicy)
    this.connect();
};

MongoDataStore.prototype.__handleReconnected = function () {
