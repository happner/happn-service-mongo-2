...
Config.prototype.parse = function(config) {
  ...
  if (!parsed.collection)
    parsed.collection =
      parsed.opts && parsed.opts.collection ? parsed.opts.collection : "happn";
...
Config.prototype.getDBConnectionUrl = function(connUrl, databaseName) {
  ...
  if (databaseName != null && databaseName != mongoUrlParts.dbName) {
    ...
    for (var i in mongoUrlParts.servers) {
      servers[i] =
        mongoUrlParts.servers[i].host + ":" + mongoUrlParts.servers[i].port;
    }
    ...
    if (connUrl.indexOf("?") !== -1)
      url += connUrl.substring(connUrl.indexOf("?"));
  }
  return url;
};
