...
function doInit(message){
  ...
  randomClient.initialize(message.config, function(e){

    if (e) {
      trySend('NOT-UP', {mode:mode, size:size});
      process.exit();
    }
...
var doGet = function(message){
  ...
  randomClient.dataStore().get(message.path, function(e, data){

    if (e) return trySend('data-get-failed', {message:message, error:e.toString()});

    trySend('data-get-complete', {message:message, data:data});
  });
};

var doSet = function(message){

  randomClient.dataStore().set(message.path, message.data, function(e, response){

    if (e) return trySend('data-set-failed', {message:message, error:e.toString()});

    trySend('data-set-complete', {message:message, response:response});
  });
};
...
process.on('message', function(serialized){
  ...
  if (message.type == 'doInit') doInit(message);
