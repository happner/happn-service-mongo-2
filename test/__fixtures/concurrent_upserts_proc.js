...
function startup(){
  ...
  serviceInstance.initialize(function(e){
    ...
    if (e) return process.send({state:'startup-error', error:e.message, type:'startup'});
...
function goCrazy(instruction){
  ...
  async.timesSeries(instruction.concurrent_attempts, function(time, timeCB){
      serviceInstance.upsert('/upsert/' + instruction.testId + '/' + time, {data: {"test" : "data"}}, {}, false,
      function(e, response, created){
        if (e) failures.push(e.message);
        timeCB();
      });
  }, function(){
    process.send({ok:failures.length == 0, failures:failures, type:'went crazy'});
  });
}
