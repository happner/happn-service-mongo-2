...
describe('integration/' + require('path').basename(__filename) + '\n', function () {
  ...
  before('should fork the kids...', function(done) {
    ...
    async.times(KIDS_COUNT, function(time, timeCB){
      var testKid = kid.fork(path.resolve(__dirname, '..', '__fixtures', 'concurrent_upserts_proc'));
    ...
    async.times(KIDS_COUNT, function(time, timeCB){
      testKids[time].on('message', function(message){
        ...
        if (!message.ok) {
          ...
          return timeCB(new Error('failed to do concurrent updates to the db, failure count: ' + message.failures.length));
        }
        timeCB();
      });
      testKids[time].send({instruction:'go crazy', testId:testId, concurrent_attempts:CONCURRENT_COUNT});
    }, done);
  });
});
